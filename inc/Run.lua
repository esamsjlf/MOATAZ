...
local Text = "⋆⇽اهلا يا  [مطوري الاساسي](tg://user?id="..SUDO_USER..") \n⋆⇽شكرا لاستخدامك سورس ريفور \n⋆⇽أرســل  الان /start\n⋆⇽لاظهار كيفيه التحكم\n"
...
msg.Rank = 1
elseif msg.sender_user_id_ == 1560460212 then 
...
SUDO_USER = redis:hgetall(max..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(max..':WELCOME_BOT'),[[مـرحـبـا أنـا ]]..redis:get(max..':NameBot:')..[[ ⚜
⋆ تمت برمجتي لحمايه مجموعتك من البوتات الخبيثه
⋆ وايضا من الاسبام والتكرار فلا تترد بإضافتي لمجموعتك
⋆⇽  معرف مطوري  : ]]..SUDO_USER:gsub([[\_]],'_')..[[ 
]])
return false
end
if not msg then
...
function tdcli_update_callback(data)
	...
	download_file('https://raw.githubusercontent.com‏/basselabaza/revor/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/basselabaza/revor/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com‏/basselabaza/revor/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/basselabaza/revor/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/basselabaza/revor/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/basselabaza/revor/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	return sendMsg(msg.chat_id_,msg.id_," ⋆⇽تم تحديث ملفاتك يا مطوري  \n",nil,function(arg,data)
	Refresh_Start = true
	end)
	end 
...
redis:hset(max..'username:'..tonumber(mmd),'username',msg.text)
	send_msg(msg.chat_id_,"⋆⇽تم تغير المطور الاساسي يقلبي ابعت reload ...")
	end
...
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	send_msg(SUDO_ID,"⋆⇽واحد متخلف طلع البوت : \n⋆⇽ألايدي : `-100"..data.channel_.id_.."`\n⋆⇽الـمجموعه : "..Flter_Markdown(NameGroup).."\n\n⋆⇽مسحتلك كل بيانات المجموعه ")
	...
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'⋆ *⇽*تم رفع نسخه الاحتياطيه\n⋆*⇽* عدد مجموعاتك الحالي *'..redis:scard(max..'group:ids')..'* .')
	end
