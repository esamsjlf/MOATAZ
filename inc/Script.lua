...
bd = '⋆ ⇽العضو ['..USERNAME..'](tg://user?id='..msg.sender_user_id_..') \n⋆⇽لأستخدام البوت عليك الاشتراك ب قناة السورس @revorb0t'
...
if res == 403 then
return "⋆ *⇽*اسف يا حب \n⋆⇽انت حاظر البوت مقدرتش ابعتلك الرابط\n!"
elseif res == 400 then
return "⋆ *⇽*اسف يا حب \n⋆⇽ حبيبي ادخل ابعتلي ماسدج عشان اقدر ابعتهولك \n!"
end
if res == 200 then 
...
local Admins = redis:scard(max..'admins:'..msg.chat_id_)
if Admins ==0 then  
return "⋆*⇽* اوه ☢ هنالك خطأ ⋆ \n⋆ ⇽عذرا لا يوجد ادمنيه عشان مسحهم " 
end
redis:del(max..'admins:'..msg.chat_id_)
...
if not msg.Director then return "⋆ *⇽*حبيبي الامر دا مش ليك دا ل {المطور,المنشئ,المدير} فقط  \n⋆ " end
if not redis:get(max..'rulse:msg'..msg.chat_id_) then 
return "⋆ ⇽عذرا لا يوجد قوانين عشان مسحه \n!" 
end
...
if NumMnsha ==0 then 
return "⋆ ⇽عذرا لا يوجد منشئيين عشان مسحهم \n!" 
end
...
local NumMDER = redis:scard(max..'owners:'..msg.chat_id_)
if NumMDER ==0 then 
return "⋆ ⇽عذرا لا يوجد مدراء عشان مسحهم \n!" 
end
redis:del(max..'owners:'..msg.chat_id_)
...
return sendMsg(msg.chat_id_,msg.id_,'⋆ ⇽معنديش صلاحيه تغيير الصوره \n⋆ ⇽يجب اعطائي صلاحيه `تغيير معلومات المجموعه ` ⠀\n')
end
end, nil)
end
...
if Total == 1 then
return sendMsg(msg.chat_id_,msg.id_,"⋆ ⇽ مفيش بوتات يحبي .") 
else
local NumBot = 0
local NumBotAdmin = 0
...
⋆ ⇽انا جيت احمي مجموعتك من التخريب
⋆ ⇽فعلني عشان اشتغل بكل مميزاتي
...
return '⋆ ⇽ اصدار سورس داني : *v'..version..'* \n⋆'
end

if (MsgText[1] == 'تحديث السورس' or MsgText[1] == 'تحديث السورس ') then
if not msg.SudoBase then return "⋆ *⇽*حبيبي الامر دا مش ليك دا ل {المطور الاساسي} فقط  \n⋆ " end
local GetVerison = https.request('https://github.com/basselabaza/danyfile/GetVersion.txt') or 0
...
send_msg(msg.sender_user_id_,"⋆| اي يحبب مش مستاهله زعلك \n⋆ ⇽ هما زعلانين انك مشيت \n\n⋆ ⇽راجع نفسك ونورنا تاني ⋆\n⋆⇽"..Flter_Markdown(redis:get(max..'group:name'..msg.chat_id_)).." :\n\n["..redis:get(max..'linkGroup'..msg.chat_id_).."]\n")
sendMsg(msg.chat_id_,msg.id_,"⋆ ⇽ لرابط في الخاص يقلبي لو احتاجت ترجع ")
else
sendMsg(msg.chat_id_,msg.id_,"⋆ ⇽ انت مشرف حبي او قافل عليا صلاحيه الحظر  ")
end
end)
return false
end
...
return [[
⋆  ━━━  𝐌𝐄𝐆𝐀 𝐓𝐑𝐎𝐍  ━━━ ⋆
  [- The Source Developer ](https://t.me/K_e_r_o_0) 💌 .
...
return sendMsg(msg.chat_id_,0,'*⋆ ⇽* انت اتحظرت من البوت عشان عامل ازعاج \n') 
end
...
SendMention(msg.sender_user_id_,data.id_,msg.id_,"⋆⇽رسالتك وصلت ⋆\n⋆ ⇽آلى : "..USERNAME.." ⋆",39,USERCAR) 
return false 
end,nil)
end  
end,nil)
...
sendMsg(msg.chat_id_,msg.id_,"⋆⇽حبيبي بعت للمطور رسالتك\n⋆⇽هيشوفها ويرد عليك علي طول\n⋆⇽معرف المطور "..SUDO_USER)
...
sendMsg(msg.chat_id_,msg.id_,"⋆⇽عندك غلط \n⋆ ⇽المعرف الذي ارسلته ليس معرف قناة.")
return false
...
⋆⇽جيت عشان احمي مجموعتد
⋆ ⇽فعلني عشان اشتغل بمميزاتي
...
GetUserID(msg.sender_user_id_,function(arg,data)  
welcome = (redis:get(max..'welcome:msg'..msg.chat_id_) or "⋆ ⇽اهلا عزيزي {الاسم}\n⋆ ⇽معرفك » {المعرف}\n{القوانين}\n\nالرجاء الالتزام بالقوانين ⋆\n⋆  ━━━  𝖘𝖔𝖓𝖎𝖈  ━━━ ⋆\n⋆⇽اسم المجموعه » {المجموعه}")
if welcome then
...
if welcome then
rules = (redis:get(max..'rulse:msg'..msg.chat_id_) or "⋆⇽مرحبأ عزيري القوانين كلاتي ⋆\n⋆ ⇽ممنوع نشر الروابط\n⋆⇽ممنوع التكلم او نشر صور اباحيه\n⋆⇽ممنوع  اعاده توجيه\n⋆⇽ممنوع التكلم الاديان كلنا اخوات\n⋆️⇽الرجاء احترام المدراء والادمنيه \n")
welcome = welcome:gsub("{القوانين}", rules)
...
if redis:get(max..'lock_woring'..msg.chat_id_) then
GetUserID(msg.sender_user_id_,function(arg,data)
local msgx = "⋆ ⇽ ممنوع التعديل تم المسح \n⋆ "
if data.username_ then USERNAME = '@'..data.username_ else USERNAME = FlterName(data.first_name_..' '..(data.last_name_ or "")) end
local USERCAR = utf8.len(USERNAME)
SendMention(msg.chat_id_,data.id_,msg.id_,"⋆ ⇽العضو » "..USERNAME..'\n'..msgx,12,USERCAR) end,nil)
end
...
if redis:get(max..'lock_woring'..msg.chat_id_) then
GetUserID(msg.sender_user_id_,function(arg,data)
local msgx = "⋆ ⇽ممنوع انك تبعت روابط  \n⋆ "
if data.username_ then USERNAME = '@'..data.username_ else USERNAME = FlterName(data.first_name_..' '..(data.last_name_ or "")) end
local USERCAR = utf8.len(USERNAME)
SendMention(msg.chat_id_,data.id_,msg.id_,"⋆ ⇽العضو » "..USERNAME..'\n'..msgx,12,USERCAR) end,nil)
end
...
if redis:get(max..'lock_woring'..msg.chat_id_) then
local msgx = "⋆ ⇽ممنوع انك تبعت معرف   \n⋆ "
GetUserID(msg.sender_user_id_,function(arg,data)
if data.username_ then USERNAME = '@'..data.username_ else USERNAME = FlterName(data.first_name_..' '..(data.last_name_ or "")) end
local USERCAR = utf8.len(USERNAME)
SendMention(msg.chat_id_,data.id_,msg.id_,"⋆ ⇽العضو » "..USERNAME..'\n'..msgx,12,USERCAR) end,nil)
...
if data.username_ then USERNAME = '@'..data.username_ else USERNAME = FlterName(data.first_name_..' '..(data.last_name_ or "")) end
local USERCAR = utf8.len(USERNAME)
SendMention(msg.chat_id_,data.id_,msg.id_,"⋆ ⇽اضغط على آلآيدي عشان آلنسـخ\n\n "..USERNAME.." ⇽ ( "..data.id_.." )",37,USERCAR)  
return false
end)
